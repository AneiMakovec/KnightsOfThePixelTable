...
- (id) initWithKnightData:(KnightData *)data area:(Rectangle *)area layerDepth:(float)layerDepth {
    ...
    if (self != nil) {
        // init pane
        equipmentPane = [[Image alloc] initWithTexture:[TownSpriteComponent getInterfaceProp:InterfacePropPane] position:[Vector2 vectorWithX:area.x + 220 y:area.y + 199]];
        equipmentPane.layerDepth = layerDepth + INTERFACE_LAYER_DEPTH_GROUNDBACK;
        [items addObject:equipmentPane];
        
        // init weapon info
        weaponLabel = [[Label alloc] initWithFont:[TownSpriteComponent getFont] text:@"Weapon" position:[Vector2 vectorWithX:area.x + 322 y:area.y + 215]];
        ...
        weapon = [[Image alloc] initWithTexture:[TownSpriteComponent getInterfaceProp:InterfacePropSlotBronze] position:[Vector2 vectorWithX:area.x + 302 y:area.y + 235]];
        weapon.layerDepth = layerDepth + INTERFACE_LAYER_DEPTH_MIDDLE;
        [items addObject:weapon];
        
        weaponNameLabel = [[Label alloc] initWithFont:[TownSpriteComponent getFont] text:@"Weapon name" position:[Vector2 vectorWithX:area.x + 322 y:area.y + 285]];
        ...
        // init armor info
        armorLabel = [[Label alloc] initWithFont:[TownSpriteComponent getFont] text:@"Armor" position:[Vector2 vectorWithX:area.x + 445 y:area.y + 215]];
        ...
        armor = [[Image alloc] initWithTexture:[TownSpriteComponent getInterfaceProp:InterfacePropSlotBronze] position:[Vector2 vectorWithX:area.x + 425 y:area.y + 235]];
        armor.layerDepth = layerDepth + INTERFACE_LAYER_DEPTH_MIDDLE;
        [items addObject:armor];
        
        armorNameLabel = [[Label alloc] initWithFont:[TownSpriteComponent getFont] text:@"Armor name" position:[Vector2 vectorWithX:area.x + 445 y:area.y + 285]];
